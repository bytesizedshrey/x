...
// Viscous spring for liquid stretch and surface tension
...
const SIZE_CLASS: Record<ExpandableActionBarSize, string> = {
  sm: "min-h-10 gap-1.5 p-1.5 text-xs",
  md: "min-h-14 gap-2 p-2 text-sm", // Thicker premium capsule
};
...
export function ExpandableActionBar({
  ...
  expandOnHover = true,
  expandOnFocus = true,
  collapseDelay = 200, // Longer delay for deliberate liquid closing
  className,
  classNames,
  renderItem,
  ...
  const spotlightX = useSpring(mouseX, { stiffness: 120, damping: 20 });
  const spotlightY = useSpring(mouseY, { stiffness: 120, damping: 20 });
  const spotlightBackground = useMotionTemplate`radial-gradient(120px circle at ${spotlightX}px ${spotlightY}px, rgba(255,255,255,0.15), transparent 100%)`;
  ...
  return (
    <LayoutGroup id={layoutId}>
      ...
      <motion.div
        animate={
          hoveredId
            ? { y: 0, scale: 1 } // Stop breathing on interaction
            : {
                y: [-2, 2, -2],
                scale: [1, 1.002, 1],
              }
      ...
      >
        <motion.div
          ...
          className={cn(
            "relative inline-flex items-center overflow-hidden rounded-[999px]",
            // Molten glass base material
            "bg-[#111111]/70 backdrop-blur-3xl saturate-150",
            // Thicker border for depth
            "ring-1 ring-inset ring-white/[0.08]",
            // Elaborate liquid lighting: drop shadow + specular top/bottom edges + ambient inner glow
            "shadow-[0_32px_64px_-16px_rgba(0,0,0,0.8),inset_0_2px_4px_rgba(255,255,255,0.1),inset_0_-4px_8px_rgba(0,0,0,0.6),inset_0_0_12px_rgba(255,255,255,0.03)]",
            SIZE_CLASS[size],
            classNames?.track,
          )}
        >
          ...
          {/* Moving reflective sheen across the capsule */}
          <motion.div
            className="absolute inset-0 pointer-events-none -z-10 rounded-full mix-blend-overlay"
            style={{
              background:
          ...
          {items.map((item) => {
            ...
            return (
              <motion.button
                ...
                transition={LIQUID_SPRING}
                className={cn(
                  "relative isolate inline-flex items-center justify-center overflow-visible rounded-full outline-none transition-all duration-500 ease-out",
              ...
              >
                {/* Liquid bubble for the active/hovered state */}
                {isHighlighted ? (
                  <motion.div
                    layoutId="action-bar-bubble"
                    transition={LIQUID_SPRING}
                    className="absolute inset-0 -z-10 rounded-full bg-white/[0.04]"
                    style={{
                      boxShadow:
                        "inset 0px 2px 4px rgba(255,255,255,0.2), inset 0px -2px 4px rgba(0,0,0,0.6), 0px 4px 12px rgba(0,0,0,0.5)",
                      backdropFilter: "blur(6px) brightness(1.05)", // Slight distortion of surrounding glass
                      border: "1px solid rgba(255,255,255,0.1)",
                    }}
                  >
                    {/* Inner subtle ripple for the bubble */}
                    <motion.div
                      ...
                      animate={
                        reduce
                          ...
                          : {
                              ...
                              opacity: isExpanded ? 1 : 0,
                              x: isExpanded ? 0 : -8,
                              marginLeft: isExpanded ? 12 : 0, // More spacing for wider capsule
                              filter: isExpanded ? "blur(0px)" : "blur(4px)",
                            }
                      }
                      transition={reduce ? { duration: 0 } : LABEL_TRANSITION}
                      className={cn(
                        "inline-block overflow-hidden whitespace-nowrap font-medium tracking-wide text-white/90",
                        classNames?.label,
                      )}
                    >
                      {item.label}
                    ...
                    <AnimatePresence>
                      {item.badge && (
                        <motion.span
                          ...
                          exit={{ opacity: 0, scale: 0.5 }}
                          className={cn(
                            "ml-0.5 inline-flex h-4 min-w-4 items-center justify-center rounded-full bg-white/20 border border-white/30 px-1 text-[9px] font-bold leading-none text-white backdrop-blur-xl shadow-[0_4px_12px_rgba(0,0,0,0.8)]",
